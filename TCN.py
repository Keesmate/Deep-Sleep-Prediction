...
# Convert 'Bed-time' and 'SunsetTime' to minutes since midnight
df['BedTimeMinutes'] = pd.to_datetime(df['Bed-time'], format='%I:%M %p').dt.hour * 60 + \
                       pd.to_datetime(df['Bed-time'], format='%I:%M %p').dt.minute

df['SunsetMinutes'] = pd.to_datetime(df['Sunset'], format='%I:%M %p').dt.hour * 60 + \
                      pd.to_datetime(df['Sunset'], format='%I:%M %p').dt.minute

# Create cyclical features for bed time (24h = 1440 minutes period)
...
df.drop(['Date', 'Bed-time', 'SunsetTime', 'BedTimeMinutes'], axis=1, inplace=True)
