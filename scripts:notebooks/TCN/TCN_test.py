...
# --- Sort saliency and keep top 20 features ---
# Get average saliency per feature
feature_saliency = saliency.mean(axis=1)
feature_importance = list(zip(feature_cols, feature_saliency))
feature_importance.sort(key=lambda x: x[1], reverse=True)

# Keep only top 20
top_features = [name for name, _ in feature_importance[:20]]
top_indices = [feature_cols.index(name) for name in top_features]
top_saliency = saliency[top_indices, :]  # shape: (20, seq_length)

# --- Plot heatmap for top 20 ---
import seaborn as sns

plt.figure(figsize=(10, 8))
sns.heatmap(top_saliency,
            xticklabels=[f"T-{SEQ_LENGTH - i - 1}" for i in range(SEQ_LENGTH)],
            yticklabels=top_features,
            cmap="viridis", annot=False)
plt.title("Top 20 Input Features by Gradient Saliency")
